...
public class MyBatisDao extends SqlSessionDaoSupport {
    ...
    }//DAOsupport必须要一个sqlsessionfactory
    /** 第一个是mapper.package,第二个是mapper.方法名,第三个是返回对象
     * 插入
     *
     * @param prefix 前缀
     * @param key 关键字
    ...
    //getsqlSession使用的是注入的sqlfactory,返回的是一个sqlsessionTemplate,所以这里里面最终调用了sqlsessionTemplate.insert()
    /**
     * 修改
     *
     * @param prefix 前缀
     ...
     * @return T 泛型结果
     */
    public <T> T  getOne(String prefix, String key) {
        return getSqlSession().selectOne(prefix + key);
    }
     ...
     * @return T 泛型结果
     */
    public <T> T getOne(String prefix, String key, Object params) {
        return getSqlSession().selectOne(prefix + key, params);
    }
